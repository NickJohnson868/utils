...
#ifdef WIN
bool CFileUtil::hide_file(const fs::path& file)
{
	return modify_file_attribute(file, FILE_ATTRIBUTE_HIDDEN, true);
}

bool CFileUtil::show_file(const fs::path& file)
{
	return modify_file_attribute(file, FILE_ATTRIBUTE_HIDDEN, false);
}

bool CFileUtil::modify_file_attribute(const fs::path& file, DWORD attribute, bool add)
{
	...
	DWORD newAttributes = add ? (attributes | attribute) : (attributes & ~attribute);
	if (newAttributes == attributes) {
		return true;
	}

    return SetFileAttributesW(file.wstring().c_str(), newAttributes);
}
#endif
...
fs::path CFileUtil::get_desktop_dir()
{
#ifdef WIN
	std::wstring desktopPath;
	const std::wstring subkey = L"SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Explorer\\Shell Folders";
	const std::wstring valueName = L"Desktop";

	if (!CRegistryUtil::read_registry_key(HKEY_CURRENT_USER, subkey, valueName, REG_SZ, desktopPath)) {
		std::wcerr << L"Failed to read desktop path from registry." << std::endl;
		return L"";
	}
    ...
    if (desktopPath.empty()) {
        desktopPath = std::string(home) + "/Desktop";
    }
    return desktopPath;
#endif
}
...
bool CFileUtil::png_to_jpg(const fs::path& pngPath, const fs::path& jpgPath) {
    std::string command = CStringUtil::format(R"(%sffmpeg -hide_banner -loglevel error -i "%s" "%s")",
...
std::string compress_command(const fs::path& filePath, int maxSize, double xScale, double yScale,
    ...
    std::string command = CStringUtil::format(
        R"(%sffmpeg -nostats -loglevel error -progress pipe:1 -hide_banner -y -i "%s" -vf "scale=%d:%d,split[s0][s1];[s0]palettegen=reserve_transparent=1[p];[s1][p]paletteuse" "%s")",
        FFMPEG_DIR,
...
bool CFileUtil::get_file_json_info(const fs::path& file, nlohmann::json& j) {
    if (!CFileUtil::is_exists(file))
        return false;
    std::string command = CStringUtil::format(
        R"("%sffprobe -v quiet -print_format json -show_streams "%s")",
        FFMPEG_DIR,
        file.string().data()
    );
...
void CFileUtil::ts_to_mp4(const fs::path& input_ts, const fs::path& output_mp4, std::string codec = "h264") {
    ...
    std::string command = CStringUtil::format(R"(%sffmpeg -y -hide_banner -loglevel error -i "%s" -c:v %s_nvenc -x265-params log-level=0 -c:a copy -progress pipe:1 -nostats "%s")",
        FFMPEG_DIR,
...
void CFileUtil::mp4_to_ts(const fs::path& input_mp4, const fs::path& output_ts) {
    ...
    if (need_transcode) {
        ...
        command = CStringUtil::format(
            R"(%sffmpeg -y -hide_banner -loglevel error -i "%s" -c:v h264_nvenc -c:a copy -f mpegts -progress pipe:1 "%s")",
            FFMPEG_DIR,
        ...
        /* 直接复制编码到 TS 格式 */
        command = CStringUtil::format(
            R"(%sffmpeg -y -hide_banner -loglevel error -i "%s" -c:v copy -c:a copy -f mpegts -progress pipe:1 "%s")",
            FFMPEG_DIR,
            input_mp4.string().data(),
            output_ts.string().data()
        );
