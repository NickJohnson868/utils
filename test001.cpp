...
namespace Test
{
	...
	LL _pow(int base, int exp)
	{
		...
		if (!_use_cache)
		{
			if (_use_quick_pow) return _Math::fast_pow(base, exp);
		...
		if (_use_quick_pow) _pow_cache[base][exp] = _Math::fast_pow(base, exp);
	...
	LL daffodils(int n, bool use_cache, bool is_sync, bool use_quick_pow, bool use_for_mult)
	{
		...
		FunctionResult<void> res = _Time::time_execution<void(int), int>(daffodils, n);
